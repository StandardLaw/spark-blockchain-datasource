...
import com.liorregev.spark.blockchain.bitcoin.transaction._
...
//      .option("enrich", "false")
        .option("network", id)
        .format("com.liorregev.spark.blockchain.bitcoin.transaction")
        .load(path)
        .as[Transaction]
    }
...
//      .option("enrich", "false")
        .option("network", id)
        .format("com.liorregev.spark.blockchain.bitcoin.io")
        .load(path)
        .as[IO]
    }
  }
