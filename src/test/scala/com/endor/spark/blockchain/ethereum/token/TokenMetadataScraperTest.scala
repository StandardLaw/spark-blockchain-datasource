...
class TokenMetadataScraperTest extends FunSuite with Matchers {
  ...
  test("Getting EOS token data from etherscan") {
    val address = "86fa049857e0209aa7d9e616f7eb3b3b78ecfdb0"
    val expectedMetadata = TokenMetadata(address, "", "EOS", "1000000000000000000000000000", Option(18))

    val scraper = new EtherscanTokenMetadataScraper()
    val result = Await.result(scraper.scrapeAddress(address), 1 minute)
  ...
  test("Getting OMGToken data from Ethplorer") {
    ...
    val expectedMetadata = TokenMetadata(address, "OMGToken", "OMG", "140245398245132780789239631", Option(18))
