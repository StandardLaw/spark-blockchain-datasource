...
import com.endor.spark.blockchain.ethereum.block.EthereumTestUtils
...
class TokenTransferEventsRelationTest extends FunSuite with EthereumTestUtils
    ...
    def addEvents(eventNames: String*): Unit = {
      registeredEvents ++=
        eventNames.map(eventName => {
          val eventPath = s"/com/endor/spark/blockchain/ethereum/events/$eventName.json"
  ...
  test("Base parse test") {
    val expected = TokenTransferEvent(
      ...
      "000000000000000000000000000000000000000000000047ffb0765e72b6bc00".bytes,
      3904411L,
  ...
  test("Using 2 hosts") {
    val expected = Seq(
      ...
      TokenTransferEvent(
        ...
        "00000000000000000000000000000000000000000000000000000489d32d6c00".bytes,
        3904412L,
